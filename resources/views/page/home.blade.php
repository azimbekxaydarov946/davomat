       ...
       <select class="form-control" name="filter" id="">
        @if (isset($month))
         ...
         @for ($i = 1; $i <= count($month); $i++)
          ...
          @endphp
          <option @if ($fil == $value) {{ 'selected' }} @endif value="{{ $value }}"> @php
        ...
        @foreach ($users as $key => $user)
         @if ($user->id != 1)
          ...
          <tr>
           ...
           <td style="width: 150px !important;">{{ $user->firstname }} {{ $user->lastname }} </td>
           ...
           @foreach ($days as $day)
            <td style="padding: 5px;" class="text-center">
             ...
             @foreach ($user_days as $user_day)
              ...
              @if ($user_day->user_id == $user->id && $user_day->day_id == $day->id)
               @if (auth()->user()->is_admin)
                ...
                <form action="{{ route('dayUserDestroy', ['id' => $user_day->id]) }}" method="Post">
                 @method('DELETE')
                 @csrf
                 <button style="width: 30px; height: 30px; padding: 0; margin: auto;" class="btn btn-danger d-inline-block">x
                 </button>
                </form>
                @php($count = true)
               @else
             ...
             @if (!$count && auth()->user()->is_admin )
              ...
              <form class=" d-flex flex-column justify-content-center align-items-center" action="{{ route('dayUserStore', ['day_id' => $day->id, 'user_id' => $user->id]) }}" method="post">
               @csrf


               <button style="width: 30px; height: 30px; padding: 0" type="submit" class="btn btn-primary ">+
               </button>
